...
class _BurdenRow extends StatelessWidget {
  ...
  /// Bar color now follows the domain's actual status (red/yellow/green
  /// for risk/moderate/normal) instead of only ever choosing between
  /// red and yellow based on `highlighted`. `highlighted` still forces
  /// red on top of that — it's meant to call out a domain regardless of
  /// its status (e.g. one tied to an active red flag).
  Color get _barColor {
    if (item.highlighted) return AppColors.burdenBarRed;
    switch (item.status) {
      case DomainStatus.risk:
        return AppColors.burdenBarRed;
      case DomainStatus.moderate:
        return AppColors.burdenBarYellow;
      case DomainStatus.normal:
